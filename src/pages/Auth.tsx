...
const phoneSchema = z.object({
  phone: z
    ...
    .regex(/^\+[1-9]\d{6,14}$/, {
      message:
        "Enter phone in E.164 format with + country code (e.g., +15551234567, +919692655795)",
    }),
  name: z.string().optional(),
});
...
const Auth = () => {
  ...
  const [otpStage, setOtpStage] = useState<
    "none" | "email_reset" | "phone_login" | "phone_signup"
  >("none");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (isLogin) {
          ...
          if (error) {
            if (
              error.message
                .toLowerCase()
                .includes("unsupported phone provider") ||
              error.message.toLowerCase().includes("phone provider")
            ) {
              toast({
                title: "Phone login not configured",
                description:
                  "SMS provider is not enabled for this project. Please use Email login or ask admin to enable Phone Auth (Twilio/Vonage) in Supabase.",
                variant: "destructive",
              });
              return;
            }
          ...
          if (error) {
            if (
              ...
              toast({
                ...
                description:
                  "SMS provider is not enabled for this project. Please use Email signup or ask admin to enable Phone Auth (Twilio/Vonage) in Supabase.",
                variant: "destructive",
              });
              return;
            }
          ...
          toast({
            title: "OTP sent",
            description:
              "We sent a verification code to your phone to complete signup.",
          });
          return;
        }
      }
      ...
      if (isLogin) {
        ...
        if (error) {
          if (error.message.includes("Invalid login credentials")) {
            ...
            toast({
              ...
              description:
                "Invalid API key. Please verify VITE_SUPABASE_URL and VITE_SUPABASE_PUBLISHABLE_KEY in your .env match the same Supabase project.",
              variant: "destructive",
            });
          } else {
            toast({
        ...
        // Sync user profile
        if (postSession?.access_token && BACKEND) {
          try {
            await fetch(`${BACKEND}/api/users/sync`, {
              method: "POST",
              headers: { Authorization: `Bearer ${postSession.access_token}` },
              credentials: "include",
            });
          } catch {}
        }
        const dest =
          redirectParam ||
          defaultDestinationFor(postSession?.user?.email ?? email.trim());
        navigate(dest);
      } else {
        ...
        toast({
          ...
          description: sessionCreated
            ? "Welcome to AcreCap. You are now logged in."
            : "Please check your email to confirm your account before logging in.",
        });
        if (sessionCreated) {
          // Sync user profile
          if (data.session?.access_token && BACKEND) {
            try {
              await fetch(`${BACKEND}/api/users/sync`, {
                method: "POST",
                headers: {
          ...
          const dest =
            redirectParam ||
            defaultDestinationFor(data?.user?.email ?? email.trim());
          navigate(dest);
        }
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          msg.includes("email not enabled")
        ) {
      ...
      toast({
        title: "Error",
        description: e?.message || "Failed to send email. Please try again.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
    ...
    try {
      if (!otpCode.trim() || !newPassword.trim()) {
        toast({
          ...
          description: "Enter the code sent to your email and a new password.",
        });
        return;
      }
      setIsLoading(true);
      ...
      toast({
        ...
        description: e?.message || "Unable to verify code or update password.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleVerifyPhoneOtp = async () => {
    try {
      ...
      // Sync user profile
      if (phoneSession?.access_token && BACKEND) {
        try {
          await fetch(`${BACKEND}/api/users/sync`, {
            method: "POST",
            headers: { Authorization: `Bearer ${phoneSession.access_token}` },
            credentials: "include",
          });
        } catch {}
      }
      const dest =
        redirectParam ||
        defaultDestinationFor(phoneSession?.user?.email ?? null);
      navigate(dest);
    } catch (e: any) {
      toast({
        title: "Verification failed",
        description: e?.message || "Unable to verify OTP.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
        ...
        <div className="w-full max-w-md">
          ...
          <div className="glass-card rounded-3xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="border border-border rounded-xl p-4 bg-secondary/30">
                  ...
                  <div className="mt-3 flex gap-2">
                    <Button
                      type="button"
                      variant="accent"
                      onClick={handleVerifyEmailReset}
                      disabled={isLoading}
                    >
                      Verify & Reset
                    </Button>
              ...
              {otpSent &&
                (otpStage === "phone_login" || otpStage === "phone_signup") && (
                  <div className="border border-border rounded-xl p-4 bg-secondary/30">
                    <h3 className="text-sm font-semibold mb-2">
                      Verify Phone OTP
                    </h3>
